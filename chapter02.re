...
削除ページの表示、及び、実際に削除する処理の二つルーティングを追加しています。どちらも/todo/:idというパスから始まるようにするために、まず@<code>{on}メソッドでブランチ(ここから"todo"パスのルーティングになります、という宣言)を作成します。また、パラメータとしてTodoのIDが必須になるので、パラメータにIntegerの値がくることを宣言するように、@<code>{on}メソッドの引数に@<code>{Integer}を指定しています。
...
昨今、ブラウザやJavaScriptライブラリの進化に伴い、サーバ側はAPIを返すだけでviewは全てJavaScriptを実装する、というアプリケーションも多くあります。また、スマートフォンアプリーション用にAPIが必要な場合も、viewは不要になることがあるでしょう。
...
まず、下記のように、JSONプラグインの読み込み処理をapp.rbに追加しましょう。

//list[app.rb][app.rb]{
plugin :json, classes: [Array, Hash, Sequel::Model]
//}

@<code>{classes}にはJSONに変換する対象のクラスを指定します。デフォルトはArrayとHashです。今回はSequelで取得した結果も変換対象にしたいので、対象に@<code>{Sequel::Model}も追加しています。
...
ファイルにルーティングを追加しましょう。/api/todosにアクセスしたら、JSONが出力されるようにしましょう。

//list[routes/api.rb][routes/api.rb]{
class MyTodo
  route 'api' do |r|
    r.get 'todos' do
      Todo.all
