...
Model-View-Controller(以降MVC)パターンにおけるController部分の機能のみを提供しています。RailsのようにMVC全ての機能を提供しているわけではありません。
...
コードについては、恐らくクラス名やメソッド名から何をしている処理なのかわかるかもしれませんが、簡単に説明します。

: Roda
  Rodaのコアとなるクラス。このクラスを継承したクラスで処理を定義す必要があります。
: route
  ここで定義したブロックがリクエスト処理の際に実行されます。ブロックはRack::Requestのサブクラスのインスタンスで実行されます。
: root
...
一見するとSinatraの方がURLと処理のマッピングわかりやすくて良いのでは、と感じられる方もいるでしょう。それはそれで正しい意見です。しかし、木構造になっている事で、ネームスペースや変数の定義をDRYにする事が出来ます。

これはパスが一階層しかないとあまり恩恵が無いかもしれませんが、パスの階層が深くなればなるだけ、よりわかりやすさに差が出てくるのではないかと筆者が考えています。
...
Rodaの大きな特徴の1つに、プラグインシステムがあります。Rodaは、各種機能をプラグインとして提供していて、コアの機能は最小限にななっています。例えばテンプレートのレンダリング処理もデフォルトでは使用出来るようになっておらず、明示的にプラグインを利用する必要があります。
...
これはRailsの原則の1つである"おまかせ"とは真逆で、使用したい料理を自分で明示的に選択する必要があります。そもそもどんな料理があるかがわからない最初のうちは、これは大変な作業かもしれません。
...
RodaがHP上でうたっている特徴の1つに、パフォーマンスがあります。HP上には、@<i>{Roda is the one of the fastest Ruby web frameworks.}という記載もあります。中々強気な発言ですね。
...
ベンチマークは複数のパターン(JSONを返す、データベースに対して1つSQLを実行する、等々)、データベースも複数パターン(MySQL、PostgreSQL、MongoDB、等々)、アプリケーションサーバも当然複数パターン(Puma、unicorn、等々)で結果を取得するようなっており、大変多岐に渡っています。
...
mrubyとCRubyの結果を混ぜるのはそもそもどうなのだろう、という意見もあるかと思いますが、そこは一旦置いておいておいてやはりmruby + HTTPサーバが圧倒的に速いですね。mrubyについで、CRubyではroda + Sequel + PostgreSQLの組み合わせが三位になっています。ついで四位もroda + Sequel(こちらはデータベースがMySQL)です。なお、どちらもアプリケーションサーバはPumaです。そしてその次にSinatra + PostgreSQLの組みあわせが来ています。bench-microほどの差は出ていないですが、それでも1.7倍程度高速なようです。
