...
詳細な挙動を確認したくなったら、やはりソースコードを見るのが確実です。Rodaのコアのファイル(roda.rb)は1000行程度しか無いので、割とすんなり読めるでしょう。

=== Giftsmas

ここからはRodaで作られたアプリケーションについてご紹介します。まずは、Giftsmas(@<href>{https://github.com/jeremyevans/giftsmas})というアプリケーションです。これはJeremy Evans氏本人が作成したアプリケーションで、ギフトを管理する為のアプリケーション(どんなイベントがあって、誰に何のギフトを送ったのか等)です。
...
最後にもう1つ。Kontena(@<href>{https://github.com/kontena/kontena/tree/master/server})という、Dockerコンテナを管理する為のアプリケーションです。
